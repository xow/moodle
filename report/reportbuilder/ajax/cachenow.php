...
require_once($CFG->dirroot.'/report/reportbuilder/dialogs/dialog_content_cachenow.class.php');
...
function forkreportcache($reportid) {
    ...
    $message = get_string('cachegenstarted', 'report_reportbuilder', userdate(time()));
    ob_start();
    switch($child) {
        case -1:
...
if ($report->cache) {
    ...
    $message = get_string('cachegenstarted', 'report_reportbuilder', userdate($start));
}
