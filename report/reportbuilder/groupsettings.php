...
$returnurl = $CFG->wwwroot . '/report/reportbuilder/groupsettings.php';
...
echo $output->single_button($CFG->wwwroot . '/report/reportbuilder/groups.php', get_string('backtoallgroups', 'report_reportbuilder'), 'get');

echo $output->heading(get_string('activitygroupingx', 'report_reportbuilder', $group->name));

echo $output->heading(get_string('assignedactivities', 'report_reportbuilder'), 3);

$info = new stdClass();
$info->count = count($activities);
$info->tag = $tag->name;
echo html_writer::tag('p', get_string('groupcontents', 'report_reportbuilder', $info));

if (count($activities)) {
    echo $output->activity_group_activities_table($activities);
}

echo $output->heading(get_string('baseactivity', 'report_reportbuilder'), 3);

$info = new stdClass();
$info->url = $CFG->wwwroot . '/mod/feedback/view.php?id=' . $baseitem->cmid;
$info->activity = $baseitem->feedback;
echo html_writer::tag('p', get_string('baseitemdesc', 'report_reportbuilder', $info));

echo $output->heading(get_string('reports', 'report_reportbuilder'), 3);
