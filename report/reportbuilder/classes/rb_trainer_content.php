...
class rb_trainer_content extends rb_base_content {
    ...
    function text_restriction($title, $reportid) {
        ...
        case 'own':
            return $title . ' ' . get_string('is', 'report_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'reports':
            return $title . ' ' . get_string('reportsto', 'report_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'ownandreports':
            return $title . ' ' . get_string('is', 'report_reportbuilder') . ' "' .
                fullname($user) . '"' . get_string('or', 'report_reportbuilder') .
                get_string('reportsto', 'report_reportbuilder') . ' "' . fullname($user) . '"';
        default:
            return $title . ' ' . get_string('isnotfound', 'report_reportbuilder');
        }
    }
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'trainer_header', get_string('showbyx',
            'report_reportbuilder', lcfirst($title)));
        $mform->setExpanded('trainer_header');
        $mform->addElement('checkbox', 'trainer_enable', '',
            get_string('showbasedonx', 'report_reportbuilder', lcfirst($title)));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerownrecords', 'report_reportbuilder'), 'own');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('trainerstaffrecords', 'report_reportbuilder'), 'reports');
        $radiogroup[] =& $mform->createElement('radio', 'trainer_who', '',
            get_string('both', 'report_reportbuilder'), 'ownandreports');
        $mform->addGroup($radiogroup, 'trainer_who_group',
            get_string('includetrainerrecords', 'report_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        $mform->addHelpButton('trainer_header', 'reportbuildertrainer', 'report_reportbuilder');
    }
