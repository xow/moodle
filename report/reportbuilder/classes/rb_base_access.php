...
class rb_role_access extends rb_base_access {
    ...
    */
    ...
    function form_template(&$mform, $reportid) {
        ...
        $mform->addElement('header', 'accessbyroles', get_string('accessbyrole', 'report_reportbuilder'));
        ...
        if (!empty($roles)) {
            $contextoptions = array('site' => get_string('systemcontext', 'report_reportbuilder'), 'any' => get_string('anycontext', 'report_reportbuilder'));

            // set context for role-based access
            $mform->addElement('select', 'role_context', get_string('context', 'report_reportbuilder'), $contextoptions);
            $mform->setDefault('role_context', $context);
            $mform->disabledIf('role_context', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('role_context', 'reportbuildercontext', 'report_reportbuilder');
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('roleswithaccess', 'report_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('roles', 'reportbuilderrolesaccess', 'report_reportbuilder');
        } else {
            $mform->addElement('html', html_writer::tag('p', get_string('error:norolesfound', 'report_reportbuilder')));
        }
