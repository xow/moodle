 ...
 */
    function __construct() {
        ...
        foreach ($properties as $property) {
            if (!property_exists($this, $property)) {
                ...
                throw new ReportBuilderException(get_string('error:propertyxmustbesetiny', 'report_reportbuilder', $a));
            }
        }
        ...
        foreach ($joindata as $joinfunction => $jointable) {
            //error_log("LINE 137 joinfunction sort: $joinfunction => $jointable");
        ...
        foreach ($this->columnoptions as $columnoption) {
            if (isset($columnoption->extrafields) && is_array($columnoption->extrafields)) {
                foreach ($columnoption->extrafields as $extrakey => $extravalue) {
                    if ($extrakey == 'id') {
                        throw new ReportBuilderException(get_string('error:columnextranameid', 'report_reportbuilder', $extravalue), 101);
                    }
                }
            }
        }
    ...
    private function validate_joinlist() {
        ...
        foreach ($joinlist as $item) {
            ...
            if (in_array($item->name, $joins_used)) {
                ...
                throw new ReportBuilderException(get_string('error:joinxusedmorethanonceiny', 'report_reportbuilder', $a));
            } else {
            ...
            if (in_array($item->name, $reserved_words)) {
                ...
                $a->join = $item->name;
                $a->source = get_class($this);
                throw new ReportBuilderException(get_string('error:joinxisreservediny', 'report_reportbuilder', $a));
            }
        }
            ...
            if (isset($item->dependencies) &&
                ...
                foreach ($item->dependencies as $dep) {
                    ...
                    if (!in_array($dep, $joins_used)) {
                        ...
                        $a->source = get_class($this);
                        $a->dependency = $dep;
                        throw new ReportBuilderException(get_string('error:joinxhasdependencyyinz', 'report_reportbuilder', $a));
                    }
                }
    ...
    function new_column_from_option($type, $value, $heading=null, $customheading = true, $hidden=0) {
        ...
        if ($coloption =
            ...
            if (!reportbuilder::check_joins($joinlist, $coloption->joins)) {
                ...
                $a->source = get_class($this);
                throw new ReportBuilderException(get_string('error:joinsfortypexandvalueynotfoundinz', 'report_reportbuilder', $a));
            }
            ...
            $a->value = $value;
            $a->source = get_class($this);
            throw new ReportBuilderException(get_string('error:columnoptiontypexandvalueynotfoundinz', 'report_reportbuilder', $a));
        }
    }
    ...
    function rb_display_nice_time_in_timezone($date, $row) {
        if ($date && is_numeric($date)) {
            if (empty($row->timezone)) {
                ...
                $tzstring = get_string('nice_time_unknown_timezone', 'report_reportbuilder');
            } else {
                $targetTZ = $row->timezone;
                $tzstring = get_string(strtolower($targetTZ), 'timezones');
            }
            $date = userdate($date, get_string('nice_time_in_timezone_format', 'report_reportbuilder'), $targetTZ) . ' ';
    ...
    function rb_display_nice_date_in_timezone($date, $row) {
        if ($date && is_numeric($date)) {
            ...
            $date = userdate($date, get_string('nice_date_in_timezone_format', 'report_reportbuilder'), $targetTZ) . ' ';
            return $date;
        } else {
            return '';
        }
    ...
    function rb_display_deleted_status($status, $row) {
        switch($status) {
            case 1:
                return get_string('deleteduser', 'report_reportbuilder');
            case 2:
                return get_string('suspendeduser', 'report_reportbuilder');
            default:
                return get_string('activeuser', 'report_reportbuilder');
        }
    }
    ...
    function rb_display_language_code($code, $row) {
        global $CFG;
        if (empty($code)) {
            return get_string('notspecified', 'report_reportbuilder');
        }
        $countries = get_string_manager()->get_list_of_languages();
        if (!array_key_exists($code, $countries)) {
            return get_string('unknown', 'report_reportbuilder');
        }
    ...
    function rb_display_user_email($email, $row, $isexport = false) {
        ...
        if ($maildisplay != 1) {
            return get_string('useremailprivate', 'report_reportbuilder');
        }
    ...
    function rb_display_grade_string($item, $row) {
        ...
        if ($item === null) {
            ...
            $a->grade = $usergrade;
            $a->pass = $passgrade;
            return get_string('gradeandgradetocomplete', 'report_reportbuilder', $a);
        }
    }
    ...
    protected function add_user_table_to_joinlist(&$joinlist, $join, $field) {
        //error_log("LINE 1308: add_user_table_to_joinlist inputs: $join.$field");
        ...
        error_log("Finished adding {user} to join list");
    }
    ...
    protected function add_user_fields_to_columns(&$columnoptions,
        $join='auser') {
        global $DB, $CFG;
        error_log("Beginning adding {user} fields to columns");
        $columnoptions[] = new rb_column_option(
            'user',
            'fullname',
            get_string('userfullname', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'namelink',
            get_string('usernamelink', 'report_reportbuilder'),
            $DB->sql_fullname("$join.firstname", "$join.lastname"),
            array(
                'joins' => $join,
                'displayfunc' => 'link_user',
                'defaultheading' => get_string('userfullname', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'namelinkicon',
            get_string('usernamelinkicon', 'report_reportbuilder'),
            $DB->sql_fullname("$join.firstname", "$join.lastname"),
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'email',
            get_string('useremail', 'report_reportbuilder'),
        ...
        if (!empty($CFG->showuseridentity) &&
            ...
            $columnoptions[] = new rb_column_option(
                'user',
                'emailunobscured',
                get_string('useremailunobscured', 'report_reportbuilder'),
                "$join.email",
                array(
                    'joins' => $join,
                    'displayfunc' => 'user_email_unobscured',
                    'defaultheading' => get_string('useremail', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'lastlogin',
            get_string('userlastlogin', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'firstaccess',
            get_string('userfirstaccess', 'report_reportbuilder'),
            "$join.firstaccess",
            array(
                'joins' => $join,
        ...
        $fields = array(
            'firstname' => get_string('userfirstname', 'report_reportbuilder'),
            'lastname' => get_string('userlastname', 'report_reportbuilder'),
            'username' => get_string('username', 'report_reportbuilder'),
            'idnumber' => get_string('useridnumber', 'report_reportbuilder'),
            'id' => get_string('userid', 'report_reportbuilder'),
            'phone1' => get_string('userphone', 'report_reportbuilder'),
            'institution' => get_string('userinstitution', 'report_reportbuilder'),
            'department' => get_string('userdepartment', 'report_reportbuilder'),
            'address' => get_string('useraddress', 'report_reportbuilder'),
            'city' => get_string('usercity', 'report_reportbuilder'),
        );
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'country',
            get_string('usercountry', 'report_reportbuilder'),
            "$join.country",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'deleted',
            get_string('userstatus', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'timecreated',
            get_string('usertimecreated', 'report_reportbuilder'),
            "$join.timecreated",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_datetime'
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'timemodified',
            get_string('usertimemodified', 'report_reportbuilder'),
            "$join.timemodified",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_datetime'
        ...
        error_log("Finished column options for {user} fields adding to columns");
        return true;
    }
    ...
    protected function add_user_fields_to_filters(&$filteroptions) {
        error_log("Beginning adding {user} fields adding to filters");
        // auto-generate filters for user fields
        $fields = array(
            'fullname' => get_string('userfullname', 'report_reportbuilder'),
            ...
            'address' => get_string('useraddress', 'report_reportbuilder'),
            'city' => get_string('usercity', 'report_reportbuilder'),
            'email' => get_string('useremail', 'report_reportbuilder'),
        );
        foreach ($fields as $field => $name) {
        ...
        $filteroptions[] = new rb_filter_option(
            ...
            'select',
            array(
                'selectchoices' => array(0 => get_string('activeonly', 'report_reportbuilder'),
                                         1 => get_string('deletedonly', 'report_reportbuilder'),
                                         2 => get_string('suspendedonly', 'report_reportbuilder')),
                'attributes' => $select_width_options,
                'simplemode' => true,
            )
        );
        ...
        error_log("Finished adding {user} fields adding to filters");
        return true;
    }
    ...
    protected function add_course_table_to_joinlist(&$joinlist, $join, $field) {
        ...
        error_log("Finished adding {course} table to joinlist");
    }
    ...
    protected function add_course_fields_to_columns(&$columnoptions, $join='course') {
        global $DB;
        error_log("Beginning adding {course} fields to columns");
        $columnoptions[] = new rb_column_option(
            'course',
            'fullname',
            get_string('coursename', 'report_reportbuilder'),
            "$join.fullname",
            array('joins' => $join)
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'courselink',
            get_string('coursenamelinked', 'report_reportbuilder'),
            "$join.fullname",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'courselinkicon',
            get_string('coursenamelinkedicon', 'report_reportbuilder'),
            "$join.fullname",
            array(
                'joins' => $join,
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'visible',
            get_string('coursevisible', 'report_reportbuilder'),
            "$join.visible",
            array(
                'joins' => $join,
                'displayfunc' => 'yes_no'
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'icon',
            get_string('courseicon', 'report_reportbuilder'),
            "$join.icon",
            array(
                'joins' => $join,
                'displayfunc' => 'course_icon',
                'defaultheading' => get_string('courseicon', 'report_reportbuilder'),
                'extrafields' => array(
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'shortname',
            get_string('courseshortname', 'report_reportbuilder'),
            "$join.shortname",
            array('joins' => $join)
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'idnumber',
            get_string('courseidnumber', 'report_reportbuilder'),
            "$join.idnumber",
            array('joins' => $join)
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'id',
            get_string('courseid', 'report_reportbuilder'),
            "$join.id",
            array('joins' => $join)
        );
        $columnoptions[] = new rb_column_option(
            'course',
            'startdate',
            get_string('coursestartdate', 'report_reportbuilder'),
            "$join.startdate",
            array(
                'joins' => $join,
                'displayfunc' => 'nice_date',
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'name_and_summary',
            get_string('coursenameandsummary', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'summary',
            get_string('coursesummary', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'course',
            'coursetypeicon',
            get_string('coursetypeicon', 'report_reportbuilder'),
            "$join.coursetype",
            array(
                'joins' => $join,
                'displayfunc' => 'course_type_icon',
                'defaultheading' => get_string('coursetypeicon', 'report_reportbuilder'),
            )
        );
        // add language option
        $columnoptions[] = new rb_column_option(
            'course',
            'language',
            get_string('courselanguage', 'report_reportbuilder'),
            "$join.lang",
            array(
                'joins' => $join,
        ...
        error_log("Finished adding {course} fields to columns");
        return true;
    }
    ...
    protected function add_course_fields_to_filters(&$filteroptions) {
        error_log("Beginning adding {course} fields to filters");
        $filteroptions[] = new rb_filter_option(
            'course',
            'fullname',
            get_string('coursename', 'report_reportbuilder'),
    ...
    protected function add_course_category_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
                'course_category',
                'name',
                get_string('coursecategory', 'report_reportbuilder'),
                "$catjoin.name",
                array('joins' => $catjoin)
        );
        $columnoptions[] = new rb_column_option(
                'course_category',
                'namelink',
                get_string('coursecategorylinked', 'report_reportbuilder'),
                "$catjoin.name",
                array(
                    'joins' => $catjoin,
                    'displayfunc' => 'link_course_category',
                    'defaultheading' => get_string('category', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
                'course_category',
                'id',
                get_string('coursecategoryid', 'report_reportbuilder'),
                "$coursejoin.category",
    ...
    protected function add_position_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisationid',
            get_string('usersorgid', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            ...
            get_string('usersorgidnumber', 'report_reportbuilder'),
            "$org.idnumber",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisationpath',
            get_string('usersorgpathids', 'report_reportbuilder'),
            "$org.path",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'organisation',
            get_string('usersorgname', 'report_reportbuilder'),
            "$org.fullname",
            array('joins' => $org)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'org_type',
            get_string('organisationtype', 'report_reportbuilder'),
            'org_type.fullname',
            array(
                'joins' => 'org_type'
            )
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'org_type_id',
            get_string('organisationtypeid', 'report_reportbuilder'),
            'organisation.typeid',
            array('joins' => $org, 'selectable' => false)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'positionid',
            get_string('usersposid', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'positionidnumber',
            get_string('usersposidnumber', 'report_reportbuilder'),
            "$pos.idnumber",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'positionpath',
            get_string('userspospathids', 'report_reportbuilder'),
            "$pos.path",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'position',
            get_string('userspos', 'report_reportbuilder'),
            "$pos.fullname",
            array('joins' => $pos)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'pos_type',
            get_string('positiontype', 'report_reportbuilder'),
            'pos_type.fullname',
            array(
                'joins' => 'pos_type'
            )
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'pos_type_id',
            get_string('positiontypeid', 'report_reportbuilder'),
            'position.typeid',
            array('joins' => $pos, 'selectable' => false)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'title',
            get_string('usersjobtitle', 'report_reportbuilder'),
            "$posassign.fullname",
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'posstartdate',
            get_string('posstartdate', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'posenddate',
            get_string('posenddate', 'report_reportbuilder'),
    ...
    protected function add_position_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'organisationid',
            get_string('participantscurrentorgbasic', 'report_reportbuilder'),
            'select',
            array(
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'organisationpath',
            get_string('participantscurrentorg', 'report_reportbuilder'),
            'hierarchy',
            array(
                'hierarchytype' => 'org',
            )
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'positionid',
            get_string('participantscurrentposbasic', 'report_reportbuilder'),
            'select',
            array(
        ...
        $filteroptions[] = new rb_filter_option(
            'user',
            'positionpath',
            get_string('participantscurrentpos', 'report_reportbuilder'),
            'hierarchy',
            array(
                'hierarchytype' => 'pos',
            )
    ...
    /*protected function add_custom_fields_for($cf_prefix, $join, $joinfield,
        ...
        //error_log("LINE 2452: add_custom_fields_for function inputs:".$cf_prefix." ".$join." ".$joinfield);
        //error_log(print_r($joinlist, true));
        
        //foreach($joinlist as $o){
            //error_log(print_r($o->name, true));
        //}
        
        $seek = false;
        foreach ($joinlist as $object) {
            //error_log("LINE 2456 object->name: $object->name");
            //error_log("LINE 2457 join: $join");
            
            $seek = ($object->name == $join);
            if ($seek) {
                break;
            } else if ($seek == false && $object->name == 'auser'){
                $seek = true;
                break;
            }
        }
        ...
        if (!$seek) {
            ...
            $a->join = $join;
            $a->source = get_class($this);
            throw new ReportBuilderException(get_string('error:missingdependencytable', 'report_reportbuilder', $a));
        }
        
        //error_log("LINE 2480 finished finding join: $join");

        // build the table names for this sort of custom field data
        if($cf_prefix == 'user' || $cf_prefix == 'auser'){
            $fieldtable = 'user_info_field';
            $datatable = 'user_info_data';
        } else {
        ...
        foreach ($items as $record) {
            ...
            if (($cf_prefix == 'auser' || $cf_prefix == 'user') && $record->visible != PROFILE_VISIBLE_ALL) {
            ...
            error_log("columnsql ".print_r($columnsql));
            ...
            error_log("datatable: $datatable");
            error_log("joinname.joinfield: $joinname.$joinfield");
            error_log("join: $join");
    ...
    protected function add_custom_fields_for($cf_prefix, $join, $joinfield,
        ...
        /*if($cf_prefix == 'user'){
            error_log("add_custom_fields_for: $cf_prefix $join $joinfield");
        }*/
        ...
        foreach ($joinlist as $object) {
            /*if($object->name == $join){
                error_log("object name == join: ".$object->name." == $join");
            }*/
            $seek = ($object->name == $join);
            if ($seek) {
                break;
    ...
    protected function add_custom_user_fields(array &$joinlist, array &$columnoptions,
        ...
        //error_log(print_r($joinlist, true));
        //error_log(print_r($columnoptions, true));
        //error_log(print_r($filteroptions, true));
        //error_log("Basetable: $basetable");
    ...
    protected function add_manager_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managername',
            get_string('usersmanagername', 'report_reportbuilder'),
        ...
        $columnoptions[] = new rb_column_option(
            'user',
            'managerfirstname',
            get_string('usersmanagerfirstname', 'report_reportbuilder'),
            "$manager.firstname",
            array('joins' => $manager)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'managerlastname',
            get_string('usersmanagerlastname', 'report_reportbuilder'),
            "$manager.lastname",
            array('joins' => $manager)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'managerid',
            get_string('usersmanagerid', 'report_reportbuilder'),
            "$manager.id",
            array('joins' => $manager)
        );
        $columnoptions[] = new rb_column_option(
            'user',
            'manageridnumber',
            get_string('usersmanageridnumber', 'report_reportbuilder'),
            "$manager.idnumber",
            array('joins' => $manager)
        );
        return true;
    ...
    protected function add_manager_fields_to_filters(&$filteroptions) {
        $filteroptions[] = new rb_filter_option(
            'user',
            'managername',
            get_string('managername', 'report_reportbuilder'),
            'text'
        );
        $filteroptions[] = new rb_filter_option(
            'user',
    ...
    protected function add_tag_fields_to_columns($type, &$columnoptions, $tagids='tagids', $tagnames='tagnames') {
        ...
        $columnoptions[] = new rb_column_option(
            'tags',
            'tagids',
            get_string('tagids', 'report_reportbuilder'),
            "$tagids.idlist",
        ...
        $columnoptions[] = new rb_column_option(
            'tags',
            'tagnames',
            get_string('tags', 'report_reportbuilder'),
            "$tagnames.namelist",
        ...
        foreach ($tags as $tag) {
            ...
            $columnoptions[] = new rb_column_option(
                'tags',
                $join,
                get_string('taggedx', 'report_reportbuilder', $name),
    ...
    protected function add_tag_fields_to_filters($type, &$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'tags',         // type
            'tagids',           // value
            get_string('tags', 'report_reportbuilder'), // label
    ...
    protected function add_cohort_user_fields_to_columns(&$columnoptions,
        ...
        $columnoptions[] = new rb_column_option(
            'cohort',
            'usercohortids',
            get_string('usercohortids', 'report_reportbuilder'),
            "$cohortids.idlist",
    ...
    protected function add_cohort_course_fields_to_columns(&$columnoptions, $cohortenrolledids='cohortenrolledcourse') {
        $columnoptions[] = new rb_column_option(
            ...
            get_string('enrolledcoursecohortids', 'report_reportbuilder'),
    ...
    protected function add_cohort_program_fields_to_columns(&$columnoptions, $cohortenrolledids='cohortenrolledprogram') {
        $columnoptions[] = new rb_column_option(
            ...
            get_string('enrolledprogramcohortids', 'report_reportbuilder'),
            "$cohortenrolledids.idlist",
            array('joins' => $cohortenrolledids, 'selectable' => false)
        );
    ...
    protected function add_cohort_user_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'usercohortids',
            get_string('userincohort', 'report_reportbuilder'),
            'cohort'
        );
        return true;
    }
    ...
    protected function add_cohort_course_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'enrolledcoursecohortids',
            get_string('courseenrolledincohort', 'report_reportbuilder'),
            'cohort'
        );
    ...
    protected function add_cohort_program_fields_to_filters(&$filteroptions) {
        ...
        $filteroptions[] = new rb_filter_option(
            'cohort',
            'enrolledprogramcohortids',
            get_string('programenrolledincohort', 'report_reportbuilder'),
            'cohort'
        );
