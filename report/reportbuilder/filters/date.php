...
class rb_filter_date extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        $objs[] =& $mform->createElement('static', null, null, get_string('dateisbetween', 'report_reportbuilder'));
        $objs[] =& $mform->createElement('text', $this->name.'daysbefore', null, 'size="2"');
        $mform->setType($this->name.'daysbefore', PARAM_INT);
        $objs[] =& $mform->createElement('static', null, null, get_string('isbeforetoday', 'report_reportbuilder'));
        $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
        ...
        $objs[] =& $mform->createElement('static', null, null, get_string('isaftertoday', 'report_reportbuilder'));
    ...
    function get_label($data) {
        ...
        if ($daysafter and $daysbefore) {
            return get_string('datelabelisdaysbetween', 'report_reportbuilder', $a);

        } else if ($daysafter) {
            return get_string('datelabelisdaysafter', 'report_reportbuilder', $a);

        } else if ($daysbefore) {
            return get_string('datelabelisdaysbefore', 'report_reportbuilder', $a);
        }
        return '';
    }
}
