 ...
 */
require_once($CFG->dirroot . '/report/reportbuilder/filters/filter_forms.php');
...
class rb_filter_type {
    ...
    private function get_filteroption($type, $value) {
        ...
        if (!$filteroption) {
            ...
            $a->value = $value;
            $a->source = $sourcename;
            debugging(get_string('error:filteroptiontypexandvalueynotfoundinz', 'report_reportbuilder', $a));
            return false;
        }
    ...
    private function get_columnoption($type, $value) {
        ...
        if (!$columnoption) {
            ...
            $a->value = $value;
            $a->source = $sourcename;
            debugging(get_string('error:columnoptiontypexandvalueynotfoundinz', 'report_reportbuilder', $a));
            return false;
        }
    ...
    private function get_joins($filteroption, $columnoption) {
        ...
        if (!reportbuilder::check_joins($this->report->src->joinlist, $joins)) {
            ...
            debugging(get_string('error:joinsforfiltertypexandvalueynotfoundinz', 'report_reportbuilder', $a));
            return false;
        }
    ...
    static function get_filter($type, $value, $advanced, $report) {
        ...
        $filename = "{$CFG->dirroot}/report/reportbuilder/filters/{$filtertype}.php";
    ...
    function get_sql_filter($data) {
        print_error('abstractmethodcalled', 'report_reportbuilder', '', 'get_sql_filter()');
    }
    ...
    function check_data($formdata) {
        print_error('abstractmethodcalled', 'report_reportbuilder', '', 'check_data()');
    }
    ...
    function setupForm(&$mform) {
        print_error('abstractmethodcalled', 'report_reportbuilder', '', 'setupForm()');
    }
    ...
    function get_label($data) {
        print_error('abstractmethodcalled', 'report_reportbuilder', '', 'get_label()');
    }
}
