    ...
    function definition() {
        ...
        foreach ($REPORT_BUILDER_EXPORT_OPTIONS as $option => $code) {
            ...
            if (($exportoptions & $code) == $code && ($option != 'fusion')) {
                $exportformatselect[$code] = get_string('export' . $option, 'report_reportbuilder');
            }
        }
        ...
        $mform->addElement('header', 'general', get_string('scheduledreportsettings', 'report_reportbuilder'));

        $mform->addElement('static', 'report', get_string('report', 'report_reportbuilder'), $report->fullname);
        if (empty($savedsearches)) {
            $mform->addElement('static', '', get_string('data', 'report_reportbuilder'),
                    html_writer::div(get_string('scheduleneedssavedfilters', 'report_reportbuilder', $report->report_url()),
                            'notifyproblem'));
        } else {
            $mform->addElement('select', 'savedsearchid', get_string('data', 'report_reportbuilder'), $savedsearches);
        }
        $mform->addElement('select', 'format', get_string('export', 'report_reportbuilder'), $exportformatselect);

        if ($exporttofilesystemenabled) {
            $exporttosystemarray = array();
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttoemail', 'report_reportbuilder'), REPORT_BUILDER_EXPORT_EMAIL);
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttoemailandsave', 'report_reportbuilder'), REPORT_BUILDER_EXPORT_EMAIL_AND_SAVE);
            $exporttosystemarray[] =& $mform->createElement('radio', 'emailsaveorboth', '',
                    get_string('exporttosave', 'report_reportbuilder'), REPORT_BUILDER_EXPORT_SAVE);
            $mform->setDefault('emailsaveorboth', $exporttofilesystem);
            $mform->addGroup($exporttosystemarray, 'exporttosystemarray',
                    get_string('exportfilesystemoptions', 'report_reportbuilder'), array('<br />'), false);
        } else {
            $mform->addElement('hidden', 'emailsaveorboth', REPORT_BUILDER_EXPORT_EMAIL);
            $mform->setType('emailsaveorboth', PARAM_TEXT);
        }

        $mform->addElement('scheduler', 'schedulegroup', get_string('schedule', 'report_reportbuilder'),
...
class scheduled_reports_add_form extends moodleform {
    function definition() {
        ...
        if (!empty($reportselect)) {
            ...
            $mform->addElement('submit', 'submitbutton', get_string('addscheduledreport', 'report_reportbuilder'));
