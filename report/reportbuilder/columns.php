...
$returnurl = new moodle_url('/report/reportbuilder/columns.php', array('id' => $id));

$report = new reportbuilder($id, null, false, null, null, true);

// include jquery
local_js();
$PAGE->requires->strings_for_js(array('saving', 'confirmcoldelete', 'hide', 'show', 'delete', 'moveup', 'movedown', 'add'), 'report_reportbuilder');
...
$jsmodule = array(
    'name' => 'report_reportbuildercolumns',
    'fullpath' => '/report/reportbuilder/columns.js',
    'requires' => array('json'));
$PAGE->requires->js_init_call('M.report_reportbuildercolumns.init', $args, false, $jsmodule);
...
if ($h !== null && isset($cid)) {
    if ($report->showhide_column($cid, $h)) {
        ...
        totara_set_notification(get_string('column_vis_updated', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        totara_set_notification(get_string('error:column_vis_not_updated', 'report_reportbuilder'), $returnurl);
    }
}
...
if ($d and $confirm) {
    ...
    if (isset($cid)) {
        if ($report->delete_column($cid)) {
            ...
            totara_set_notification(get_string('column_deleted', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            totara_set_notification(get_string('error:column_not_deleted', 'report_reportbuilder'), $returnurl);
        }
    }
}
...
if ($d) {
    ...
    if (isset($cid)) {
        $confirmurl = new moodle_url('/report/reportbuilder/columns.php', array('d' => '1', 'id' => $id, 'cid' => $cid, 'confirm' => 'l', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('confirmcolumndelete', 'report_reportbuilder'), $confirmurl, $returnurl);
    }
...
if ($m && isset($cid)) {
    if ($report->move_column($cid, $m)) {
        ...
        totara_set_notification(get_string('column_moved', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        totara_set_notification(get_string('error:column_not_moved', 'report_reportbuilder'), $returnurl);
    }
}
...
if ($fromform = $mform->get_data()) {
    ...
    if (build_columns($id, $fromform, $report)) {
        ...
        totara_set_notification(get_string('columns_updated', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        totara_set_notification(get_string('error:columns_not_updated', 'report_reportbuilder'), $returnurl);
    }
