...
$returnurl = $CFG->wwwroot . '/report/reportbuilder/index.php';
...
if ($d && $confirm) {
    ...
    if (delete_report($id)) {
        ...
        totara_set_notification(get_string($type . 'report', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));

    } else {
        totara_set_notification(get_string('no' . $type . 'report', 'report_reportbuilder'), $returnurl);
    }
} else if ($d) {
    echo $output->header();
    echo $output->heading(get_string('reportbuilder', 'report_reportbuilder'));
    if ($em) {
        $continueurl = new moodle_url('/report/reportbuilder/index.php', array('id' => $id, 'd' => '1', 'em' => $em,
            'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('reportconfirm'.$type, 'report_reportbuilder'), $continueurl, $returnurl);
    } else {
        $continueurl = new moodle_url('/report/reportbuilder/index.php', array('id' => $id, 'd' => '1', 'em' => $em,
            'confirm' => '1', 'sesskey' => $USER->sesskey));
        echo $output->confirm(get_string('reportconfirm'.$type, 'report_reportbuilder'), $continueurl, $returnurl);
    ...
    if ($cache) {
        ...
        totara_set_notification(get_string('reportcacheinitialize', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        totara_set_notification(get_string('error:reportcacheinitialize', 'report_reportbuilder'), $returnurl);
    }
}
...
if ($fromform = $mform->get_data()) {
    ...
    if (empty($fromform->submitbutton)) {
        totara_set_notification(
            get_string('error:unknownbuttonclicked', 'report_reportbuilder'),
            $returnurl);
    }
    ...
    try {
        ...
        redirect($CFG->wwwroot . '/report/reportbuilder/general.php?id='.$newid);
    } catch (ReportBuilderException $e) {
        $transaction->rollback($e);
        trigger_error($e->getMessage(), E_USER_WARNING);
    } catch (Exception $e) {
        $transaction->rollback($e);
        redirect($returnurl, get_string('error:couldnotcreatenewreport', 'report_reportbuilder'));
    }
}
...
echo $output->heading(get_string('usergeneratedreports', 'report_reportbuilder'));
...
// Embedded reports
echo $output->heading(get_string('embeddedreports', 'report_reportbuilder'));
