 ...
 */
    ...
    require_once($CFG->dirroot . '/report/reportbuilder/groupslib.php');
    require_once($CFG->dirroot . '/report/reportbuilder/groups_forms.php');
    ...
    $returnurl = $CFG->wwwroot . '/report/reportbuilder/groups.php';
    ...
    if ($d && $confirm) {
        ...
        if (delete_group($id)) {
            totara_set_notification(get_string('groupdeleted', 'report_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            totara_set_notification(get_string('error:groupnotdeleted', 'report_reportbuilder'), $returnurl);
        }
    } else if ($d) {
        ...
        if ($reports) {
            ...
            totara_set_notification(get_string('error:grouphasreports', 'report_reportbuilder'), $returnurl);
            die;
        } else {
            // prompt to delete
            echo $output->header();
            echo $output->heading(get_string('activitygroups', 'report_reportbuilder'));
            echo $output->confirm(get_string('groupconfirmdelete', 'report_reportbuilder'),
                new moodle_url('/report/reportbuilder/groups.php',
    ...
    if ($fromform = $mform->get_data()) {
        ...
        if ($newid = create_group($fromform, $errorcode)) {
            redirect($CFG->wwwroot . '/report/reportbuilder/groupsettings.php?id=' .
                $newid);
            die;
        } else {
            totara_set_notification(get_string($errorcode, 'report_reportbuilder'), $returnurl);
            die;
        }
    ...
    echo html_writer::tag('p', get_string('activitygroupdesc', 'report_reportbuilder'));
