...
Feature: Configure tool types
  ...
  Background:
    ...
    And I navigate to "Manage external tools" node in "Site administration > Plugins > Activity modules > LTI"
  ...
  Scenario: Try to add a non-existant cartridge
    ...
    Then I should see "Enter your consumer key and shared secret"
    And I press "Done"
    And I should see "Unable to create tool type"
  ...
  Scenario: Attempt to add a tool type from a configuration URL, then cancel
    ...
    #When I set the field "url" to "http://lti.tools/test/tp.php"
    And I press "Add"
