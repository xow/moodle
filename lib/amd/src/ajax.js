...
define(['jquery', 'core/config', 'core/log'], function($, config, Log) {
    ...
    var requestSuccess = function(responses) {
        ...
        // In the case of a php exception, only one response will be given.
        if (typeof responses == 'object' && responses.hasOwnProperty('error')) {
            exception = responses;
        } else {
     ...
     * @param {string} exception The error thrown.
     */
    var requestFail = function(jqXHR, textStatus, exception) {
        ...
        for (i = 0; i < requests.length; i++) {
            ...
            if (unloading) {
                ...
                request.deferred.reject(textStatus, exception);
            }
        }
    };
